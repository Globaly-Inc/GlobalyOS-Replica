...
const MAX_NOTIFICATIONS_PER_IP_PER_HOUR = 30;
...
serve(async (req: Request) => {
  ...
  console.log('Leave notification request from IP:', clientIP);
  ...
  try {
    ...
    console.log(`Authorized user ${user.id} for leave notification`);
    ...
    const { count: ipRequestCount } = await supabaseClient
      ...
      .eq('attempt_type', 'leave_notify')
      .gte('created_at', oneHourAgo);

    if (ipRequestCount !== null && ipRequestCount >= MAX_NOTIFICATIONS_PER_IP_PER_HOUR) {
