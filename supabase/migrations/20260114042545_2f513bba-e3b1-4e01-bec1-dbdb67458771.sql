-- Drop the OLD function with the original parameter order (workflow_type before target_date)
-- This signature (UUID, UUID, TEXT, DATE, UUID) matches the old function where p_workflow_type came before p_target_date
DROP FUNCTION IF EXISTS public.create_workflow_from_template(UUID, UUID, TEXT, DATE, UUID);

-- Recreate the CORRECT function with stage_id fallback logic
-- Parameter order: p_employee_id, p_organization_id, p_target_date, p_workflow_type, p_created_by
...
BEGIN
  ...
  -- Get the first stage for this template (used as default for tasks without stage_id)
  ...
  -- Create tasks from template, using first stage as default if stage_id is NULL
  ...
  LOOP
    ...
    -- Use COALESCE to default to first stage if task has no stage_id
    ...
    VALUES (v_workflow_id, p_organization_id, p_employee_id, v_task.title, v_task.description, v_task.category, v_assignee_id, v_due_date, v_task.is_required, 'pending', v_task.sort_order, COALESCE(v_task.stage_id, v_first_stage_id));
  END LOOP;
