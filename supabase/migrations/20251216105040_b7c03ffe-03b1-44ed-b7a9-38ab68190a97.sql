-- Fix the notify_targeted_kudos trigger to handle each junction table correctly
...
BEGIN
  ...
  -- Handle each table type based on TG_TABLE_NAME
  FOR recipient IN
    ...
    WHERE k.id = _kudos_id
      ...
      AND e.status = 'active'
      AND (
        (TG_TABLE_NAME = 'kudos_offices' AND e.office_id = NEW.office_id)
        OR (TG_TABLE_NAME = 'kudos_departments' AND e.department = NEW.department)
