-- Add multi-session and early checkout settings to organizations table
ALTER TABLE public.organizations
ADD COLUMN IF NOT EXISTS multi_session_enabled boolean DEFAULT true,
ADD COLUMN IF NOT EXISTS max_sessions_per_day integer DEFAULT 3,
ADD COLUMN IF NOT EXISTS early_checkout_reason_required boolean DEFAULT true;

-- Add early checkout reason to attendance_records
ALTER TABLE public.attendance_records
ADD COLUMN IF NOT EXISTS early_checkout_reason text;

-- Update validate_qr_and_record_attendance function to handle multi-session settings
