-- Drop the unique constraint that's blocking multiple check-ins per day
ALTER TABLE public.attendance_records DROP CONSTRAINT IF EXISTS attendance_records_employee_id_date_unique;
ALTER TABLE public.attendance_records DROP CONSTRAINT IF EXISTS attendance_records_employee_id_date_key;

-- Drop any unique index on employee_id and date
DROP INDEX IF EXISTS attendance_records_employee_id_date_unique;
DROP INDEX IF EXISTS attendance_records_employee_id_date_key;
DROP INDEX IF EXISTS attendance_records_employee_id_date_idx;
...
BEGIN
  ...
  ELSIF _action = 'check_out' THEN
    ...
    -- Update the record with check-out time and calculate work hours
