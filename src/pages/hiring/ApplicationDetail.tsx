        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          <Card className="overflow-hidden">
            ...
            <CardContent className="p-4 space-y-3">
              <DetailRow icon={User} label="Full Name" value={candidate?.name || '—'} />
              <DetailRow
                icon={Mail}
                label="Email"
              ...
              {salaryExpectation && (
                <DetailRow icon={DollarSign} label="Salary Expectation" value={salaryExpectation} highlight />
              )}
              {candidate?.linkedin_url && (
                <DetailRow
                  icon={Linkedin}
                ...
                <>
                  ...
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`h-4 w-4 ${star <= application.rating! ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground/30'}`}
                      />
                    ))}
