...
export async function sendKpiNotifications(params: SendKpiNotificationParams): Promise<void> {
  ...
  try {
    ...
    if (scopeType === 'individual' && targetEmployeeId) {
    ...
    } else if (scopeType === 'department' && scopeDepartment) {
      // Department KPI - notify all employees in that department
      ...
      // Office KPI - notify all employees in that office
      const { data: employees } = await supabase
        .from('employees')
        .select('user_id')
        .eq('organization_id', organizationId)
      ...
      // Project KPI - notify all employees assigned to that project
