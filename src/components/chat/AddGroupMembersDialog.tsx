...
import { useAddGroupMembers } from "@/services/useChat";
...
const AddGroupMembersDialog = ({
  ...
  const { data: employees = [], isLoading: loadingEmployees } = useQuery({
    queryKey: ['employees-for-group', currentOrg?.id],
    queryFn: async () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showErrorToast(error, "Failed to add members", {
        componentName: "AddGroupMembersDialog",
        actionAttempted: "Add group members",
        errorType: "database",
      });
    }
  };
          ...
          <ScrollArea className="h-[300px]">
            {loadingEmployees ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {search ? "No matching team members" : "All team members are already in this group"}
              </div>
            ) : (
