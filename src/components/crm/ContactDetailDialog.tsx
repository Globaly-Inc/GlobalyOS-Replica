...
import { Mail, Phone, Building2, MapPin, Calendar, Flame, Handshake, Snowflake, Tag } from 'lucide-react';
...
export const ContactDetailDialog = ({ contactId, onClose }: Props) => {
  const { data: contact, isLoading } = useCRMContact(contactId);
  const { data: activities = [] } = useCRMActivities(contactId);
  ...
  const handleAddNote = () => {
    ...
    createActivity.mutate(
      { contact_id: contactId, type: 'note', content: noteText },
      {
  ...
  const address = contact ? [contact.address_street, contact.address_city, contact.address_state, contact.address_postcode, contact.address_country].filter(Boolean).join(', ') : null;
            ...
            <div className="w-full md:w-80 shrink-0 border-b md:border-b-0 md:border-r border-border p-6 overflow-y-auto">
              <div className="flex flex-col items-center text-center mb-6">
                <Avatar className="h-20 w-20 mb-3">
                  ...
                  <AvatarFallback className="text-lg">{contact.first_name?.[0]}{contact.last_name?.[0]}</AvatarFallback>
                </Avatar>
                ...
                <div className="mt-2"><RatingBadge rating={contact.rating} /></div>
              </div>

              <div className="space-y-1 border-t border-border pt-4">
                <InfoRow icon={Mail} label="Email" value={contact.email} />
                <InfoRow icon={Phone} label="Phone" value={contact.phone} />
                <InfoRow icon={Building2} label="Company" value={contact.company?.name || null} />
                <InfoRow icon={MapPin} label="Address" value={address} />
                <InfoRow icon={Calendar} label="Date of Birth" value={contact.date_of_birth ? format(new Date(contact.date_of_birth), 'dd MMM yyyy') : null} />
              </div>
              ...
              {contact.source && (
                <div className="border-t border-border pt-4 mt-4">
                  ...
                  <Badge variant="outline" className="mt-1 text-xs">{contact.source}</Badge>
                </div>
              )}
            </div>
