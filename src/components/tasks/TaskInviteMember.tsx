...
type TaskPermissionLevel = 'view' | 'edit' | 'admin';

interface TaskInviteMemberProps {
  employees: Employee[];
  offices: Office[];
  departments: string[];
  projects: Project[];
  ...
  onAdd: (selections: Selection[], permission: TaskPermissionLevel) => void;
  isAdding: boolean;
}

export const TaskInviteMember = ({
  employees,
  offices,
  departments,
  projects,
  ...
  onAdd,
  isAdding,
}: TaskInviteMemberProps) => {
  const [selections, setSelections] = useState<Selection[]>([]);
  const [permission, setPermission] = useState<TaskPermissionLevel>('edit');
  const [searchOpen, setSearchOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");

  const employeeIds = useMemo(() => employees.map(e => e.id), [employees]);
  const { data: onlineStatuses = {} } = useTeamPresence(employeeIds);

  const employeeCountByOffice = useMemo(() => {
    const counts: Record<string, number> = {};
    employees.forEach(emp => {
      if (emp.office_id) counts[emp.office_id] = (counts[emp.office_id] || 0) + 1;
    });
    return counts;
  }, [employees]);
  ...
  const employeeCountByDepartment = useMemo(() => {
    const counts: Record<string, number> = {};
    employees.forEach(emp => {
      if (emp.department) counts[emp.department] = (counts[emp.department] || 0) + 1;
    });
    return counts;
  }, [employees]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return {
      showEveryone: !selections.some(s => s.type === 'everyone') &&
      ...
      members: availableEmployees.filter(emp =>
          ...
          >
            {selections.map((sel, idx) => (
              ...
              >
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleRemoveSelection(sel); }}
                    ...
                    {filteredItems.offices.map(office => (
                      ...
                      >
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">{employeeCountByOffice[office.id] || 0} members</span>
                        </div>
                      </CommandItem>
                    ))}
                  </CommandGroup>
                    ...
                    {filteredItems.departments.map(dept => (
                      ...
                      >
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">{employeeCountByDepartment[dept] || 0} members</span>
                        </div>
                      </CommandItem>
                    ))}
                  </CommandGroup>
                    ...
                    {filteredItems.projects.map(project => (
                      ...
                      >
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">{employeeCountByProject[project.id] || 0} members</span>
                        </div>
                      </CommandItem>
                    ))}
                  </CommandGroup>
      ...
      <div className="flex items-center gap-2 w-full">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start">
            <DropdownMenuItem onClick={() => setPermission('view')} className={cn(permission === 'view' && "bg-muted")}>
              can view
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setPermission('edit')} className={cn(permission === 'edit' && "bg-muted")}>
              can edit
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setPermission('admin')} className={cn(permission === 'admin' && "bg-muted")}>
              can admin
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
