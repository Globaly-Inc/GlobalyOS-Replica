...
interface TaskTransferOwnershipDialogProps {
  open: boolean;
...
export const TaskTransferOwnershipDialog = ({
  open,
  onOpenChange,
  entityType,
  entityName,
...
}: TaskTransferOwnershipDialogProps) => {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                Transfer "{entityName}" to another team member
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            ...
            <div className="text-sm text-amber-800 dark:text-amber-200">
              ...
              <p className="text-muted-foreground mt-1">
                The new owner will have full control over this {ENTITY_LABELS[entityType]}, including the ability to delete it or transfer ownership again.
              </p>
            </div>
          </div>
                ...
                filteredEmployees.map((emp) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedEmployee?.id === emp.id ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/50"
                    )}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{emp.profiles?.full_name || "Unknown"}</p>
                      <p className="text-xs text-muted-foreground truncate">{emp.profiles?.email || ""}</p>
                    </div>
                    {selectedEmployee?.id === emp.id && <div className="h-2 w-2 rounded-full bg-primary" />}
                  </button>
                ))
              )}
            </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          <Button variant="outline" onClick={handleClose} disabled={isTransferring}>Cancel</Button>
          <Button onClick={handleTransfer} disabled={!selectedEmployee || isTransferring} className="gap-2">
            {isTransferring ? (
