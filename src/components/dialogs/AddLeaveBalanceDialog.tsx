...
export const AddLeaveBalanceDialog = ({
  ...
  const loadLeaveTypes = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("leave_types").select("id, name, category").eq("organization_id", currentOrg.id).eq("is_active", true).order("name");
    if (!error && data) {
      setLeaveTypes(data);
    }
  };
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } = await supabase.from("leave_type_balances").select("id, balance").eq("employee_id", employeeId).eq("leave_type_id", leaveType).eq("year", currentYear).maybeSingle();
      ...
      if (existingBalance) {
        const {
          error: updateError
        } = await supabase.from("leave_type_balances").update({
          balance: newBalance
        ...
        const {
          error: insertError
        } = await supabase.from("leave_type_balances").insert({
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select value={leaveType} onValueChange={setLeaveType}>
                ...
                <SelectContent>
                  {leaveTypes.length === 0 ? <SelectItem value="none" disabled>No leave types configured</SelectItem> : leaveTypes.map(lt => <SelectItem key={lt.id} value={lt.id}>
                        {lt.name} ({lt.category})
                      </SelectItem>)}
                </SelectContent>
              </Select>
              {leaveTypes.length === 0 && <p className="text-xs text-muted-foreground">
                  Configure leave types in Settings → Leave
                </p>}
            </div>
            <div className="grid gap-2">
              <Label htmlFor="amount">Days to Add/Deduct *</Label>
              <Input id="amount" type="number" step="0.5" placeholder="e.g., 5 or -2" value={amount} onChange={e => setAmount(e.target.value)} />
            ...
            <div className="grid gap-2">
              ...
              <Textarea id="reason" placeholder="e.g., Annual leave allocation, Carry forward from last year" value={reason} onChange={e => setReason(e.target.value)} rows={3} />
            </div>
          </div>
          <DialogFooter>
